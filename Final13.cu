...
// Subpart b:
// Compute capability 2.0+ GPUs have support for 3per-war p instructions.
...
__device__ unsigned int warp_reduce(unsigned int p, volatile unsigned int * s) {
	...
	//Storing value using a variable
	//The integer returned by ballot stores the p in a reversed order
	unsigned int test=__ballot(p);
