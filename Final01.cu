...
__global__ void smooth_shared(float * v_new, const float * v) {
    //s has the size of BLOCK_SIZE+2 to store additional head and end value
    extern __shared__ float s[];
    float* v_new_s=&s[blockDim.x+2];
    
    int index=threadIdx.x;
    int x=threadIdx.x+blockIdx.x*blockDim.x;

    int leftIn=(x==0)?(0):(x-1);
    int rightIn=(x==(blockDim.x*gridDim.x-1))?(x):(x+1);
    if(index==0)
    {
        s[0]=v[leftIn];
    }
    ...
    {
        s[blockDim.x+1]=v[rightIn];
    }
    s[index+1]=v[x];
    __syncthreads();
    v_new_s[index]=0.25f*s[index]+0.5f*s[index+1]+0.25f*s[index+2];
    __syncthreads();
    v_new[x]=v_new_s[index];
...
int main(int argc, char **argv)
{
    ...
    int sharedMem=(BLOCK_SIZE*2+2)*sizeof(float);
    GpuTimer timer;
    timer.Start();
    smooth_shared<<<ARRAY_SIZE / BLOCK_SIZE, BLOCK_SIZE, sharedMem>>>(d_out_shared, d_in);
    timer.Stop();
